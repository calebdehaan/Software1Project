	...
	public void recordAll() throws SQLException {
		...
		while (!stack.isEmpty()) {
			...
			if (stack.getFirst().getActionName().equals(ActionEnum.PASSCOMPLETE)) {
				statement.execute(
						"update theprojectdata.player set passes = passes + 1, completions = completions + 1 where idPlayer = \'"
								+ stack.getFirst().getId() + "\';");
			} else if (stack.getFirst().getActionName().equals(ActionEnum.PASSFAIL)) {
				statement.execute("update theprojectdata.player set passes = passes + 1 where idPlayer = \'"
						+ stack.getFirst().getId() + "\';");
			} else if (stack.getFirst().getActionName().equals(ActionEnum.INJURY)) {
				statement.execute("update theprojectdata.player set injured = '1' where idPlayer = \'"
						+ stack.getFirst().getId() + "\';");
			} else if (stack.getFirst().getActionName().equals(ActionEnum.CATCH)) {
				statement.execute("update theprojectdata.player set catches = catches + 1 where idPlayer = \'"
						+ stack.getFirst().getId() + "\';");
			}
			stack.removeFirst();
		}
